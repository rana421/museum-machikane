...
import os
os.chdir(r"C:\Users\tanigutirei\Desktop\javascript\Flask_test2")
import yojizyukugo
...
def create_room():
  ...
  for room in room_dict:
    if room == data_dict['room_name']:
      return 'そのルーム名は既に使用されています！'

  
  room_dict[data_dict['room_name']] = room_data(data_dict['room_name'],data_dict['password'],data_dict["ID"])
...
def find_room():
    ...
    for room in room_dict:
      ...
      if room == data_dict['room_name'] and room_dict[room].password == data_dict['password']:
        for a in room_dict[room].players:
...
class room_data:
  ...
  def __init__(self,room_name,password,player_name):
    ...
    self.players = [
    {"name" :"", "score":0,"change_flag":0,"TorF":"","answer_num":0,"comment":"","_isExist":0,"f_for_js":0},
    {"name" :"", "score":0,"change_flag":0,"TorF":"","answer_num":0,"comment":"","_isExist":0,"f_for_js":0},
    {"name" :"", "score":0,"change_flag":0,"TorF":"","answer_num":0,"comment":"","_isExist":0,"f_for_js":0},
    {"name" :"", "score":0,"change_flag":0,"TorF":"","answer_num":0,"comment":"","_isExist":0,"f_for_js":0}
  ...
  async def send(self,websocket,my_id):
    ...
    #↓のforは新規参加初めての操作　それ以降はすべてwhileでループ
    for player in self.players:  
      ...
      try:
        ...
        for i in range(0,4):
          if self.players[i]["_isExist"] ==1:
            break #一人でもいれば操作を続ける
          if i==3:
            del room_dict[self.room_name]
            print(self.room_name +"のルームを消去しました")
...
async def accept(websocket, path):
  ...
  for room in room_dict:
    #print(room.room_name ,"のplayers:",room.players)
    if room == room_name and room_dict[room].password ==password:
      #print("websocket_accept受信, ルーム名は"+room_name+",idは"+my_id +",パスワードは" +password)
      room_dict[room].start(websocket,my_id)
